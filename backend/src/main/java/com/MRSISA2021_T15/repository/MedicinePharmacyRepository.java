...
public interface MedicinePharmacyRepository extends CrudRepository<MedicinePharmacy, Integer> {
	...
	@Query("select mp from MedicinePharmacy mp where mp.pharmacy.id = ?1 AND mp.medicine.medicineCode like ?2")
	public MedicinePharmacy getPharmacyByIdAndMedicineCode(Integer pharmacyId, String medicineCode);
